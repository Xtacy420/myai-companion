...
import type { Memory, CheckIn } from "@/lib/database/schema";
...
function ProfilePageContent() {
  ...
  const loadProfileData = async () => {
    ...
    const [userMemories, userCheckIns] = await Promise.all([
      localDB.getMemoriesByUser(userId),
      localDB.getCheckInsByUser(userId, 5)
    ]);
  ...
  const checkIns = useQuery(
    api.checkIns.getCheckInsByUser,
    userId ? { userId } : "skip"
  );
                ...
                <CardContent>
                  <div className="space-y-4">
                    {memories?.slice(0, 5).map((memory) => (
                      <div key={memory._id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
