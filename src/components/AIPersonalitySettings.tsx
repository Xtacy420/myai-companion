...
export default function AIPersonalitySettings({ userId }: AIPersonalitySettingsProps) {
  ...
  const { user, isLoading, refreshData } = useLocalUser();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await localDB.updateUser(userId, { aiPersonality: newPersonality });

      await refreshData(); // Refresh user context to reflect changes
      setHasChanges(false);
      setIsOpen(false);
    } catch (error) {
