...
import type { LifeTemplate, CustomCalendar, Event as MyAiEvent } from "@/lib/database/schema";
import { Plus, Edit, Trash2, Repeat, Play, Info, DollarSign, Heart, Settings, Home, Briefcase, Zap, Clock, Target, Pause } from "lucide-react";
...
export default function LifeTemplateManager({ onClose }: LifeTemplateManagerProps) {
  ...
  const resetForm = () => {
    setFormData({
        ...
        template: {
            ...
            defaultReminders: [{ minutes: 1440, type: "notification" }],
            metadata: {
                amount: undefined,
                provider: "",
                accountNumber: "",
  ...
  const handleSaveTemplate = async () => {
    if (!userId || !formData.name.trim() || !formData.template.title.trim()) return;
    ...
    try {
        ...
        if (editingTemplate) {
            await localDB.updateLifeTemplate(editingTemplate.id, templateData);
        } else {
            const newTemplate: Omit<LifeTemplate, 'id'> = {
                userId,
                ...templateData,
                createdAt: Date.now(),
            };
            await localDB.createLifeTemplate(newTemplate as LifeTemplate);
        }
  ...
  const executeAutomation = async () => {
    if (!selectedTemplate || !automationSettings.calendarId || !userId) return;

    try {
        let currentDate = new Date(automationSettings.startDate);

        for (let i = 0; i < automationSettings.numberOfOccurrences; i++) {
            const newEvent: MyAiEvent = {
                ...
                status: 'pending',
                priority: 'medium',
                recurring: selectedTemplate.template.recurring,
                amount: selectedTemplate.template.metadata?.amount,
                createdAt: Date.now(),
                updatedAt: Date.now(),
            };
            await localDB.createEvent(newEvent);

            // Increment date for next event
            const { frequency, interval, daysOfWeek, dayOfMonth } = selectedTemplate.template.recurring;
            if (frequency === 'daily') {
  ...
  const initializeDefaultTemplates = async () => {
    // This would ideally be a set of predefined templates.
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {(!templates || templates.length === 0) && (
            <Button variant="outline" size="sm" onClick={initializeDefaultTemplates} className="gap-2">
      ...
      <div className="space-y-3">
        {templates && templates.length > 0 ? (
          templates.map((template) => {
            ...
            return (
              <Card key={template.id} className={`transition-colors ${template.isActive ? "" : "opacity-75"}`}>
